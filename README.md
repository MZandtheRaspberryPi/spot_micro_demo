## Spot Micro Robot
This was a fun project where I 3d printed the robot, planned the hardware, assembled, and used a fellow robot builder's (ROS Repo)[https://github.com/mike4192/spotMicro] to get it walking. I contributed a bit to the robot builder's ROS repo as well. You can see a video demo [here](https://www.youtube.com/watch?v=g6FMSfrcahk).  
...
Florian does an excellent job showing the math here. We can solve for F using pythagorean's theorem. We know L1, because we can measure this part and it doesn't change its length when spot is moving. It's plastic. Once we have F and L1, we can solve for E, again using pythagorean's theorem. We can solve for Alpha angle if we know E and L1 lengths, by using tangent. The tangent of an angle is equal to opposite divided by adjacent, so if we know E and L1 we can use arctangent to solve for the angle. Thus we solve for Alpha using arctan, and then solve for beta using arc tan on the triangle constructed with x and Y coords. Then we can subtract beta and alpha to get to the shoulder servo angle:   
